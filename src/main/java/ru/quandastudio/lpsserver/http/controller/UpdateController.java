...
public class UpdateController {
    ...
    public ResponseEntity<ByteArrayResource> getDictionary(@PathVariable(value = "version") String version) {
        return getDictionaryCommon(version, 1);
    }

    @RequestMapping(value = "/data-{version}.db2", produces = MediaType.APPLICATION_OCTET_STREAM_VALUE)
    public ResponseEntity<ByteArrayResource> getDictionaryV2(@PathVariable(value = "version") String version) {
        return getDictionaryCommon(version, 2);
    }

    private ResponseEntity<ByteArrayResource> getDictionaryCommon(String version, int structVersion) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Cache-Control", "no-cache, no-store, must-revalidate");
        headers.add("Pragma", "no-cache");
        headers.add("Expires", "0");

        return context.getDictionary().getDatabaseFile(version, structVersion).flatMap((File path) -> {
            try {
