...
public class WordMessageHandler extends MessageHandler<LPSWord> {
	...
	public void handle(Player player, LPSWord msg) {
		...
		if (firstError.isPresent()) {
			player.sendMessage(new LPSWordMessage(WordResult.NO_WORD, msg.getWord(),player.getUser().getId()));
			player.sendMessage(new LPSMsgMessage(firstError.get(), true, player.getUser().getId()));
			return;
		}
